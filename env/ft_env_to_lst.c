...
/*   Updated: 2022/03/01 22:14:52 by madorna-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	ft_env_to_lst(t_mini *mini)
{
	...
	while (mini->env[++i])
	{
		...
		if (env_node->split_mem[0])
			env_node->key = ft_strdup(env_node->split_mem[0]);
		if (env_node->split_mem[1])
			env_node->value = ft_strdup(env_node->split_mem[1]);
		ft_lstadd_back(&mini->l_env, ft_lstnew(env_node));
	}
